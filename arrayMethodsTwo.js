...
// CODE HERE
const citiesEndingWithI = cities.filter((city) => city.endsWith("i"));

console.log(citiesEndingWithI);
...
fruits.forEach((fruit, index) => {
  if (fruit.color === "red") {
    ...
    console.log(`The fruit with index ${index} is neither an apple nor an orange.`);
  }
});
...
//The fruit with index 0 is neither an apple nor an orange.
//The fruit with index 1 is an orange.
//The fruit with index 2 is neither an apple nor an orange.
//The fruit with index 3 is an apple.
...
// CODE HERE
const totalPriceOfRiceFoods = foods.reduce((totalPrice, food) => {
  if (food.tags.includes("rice")) {
    return totalPrice + food.price;
  }
  return totalPrice;
}, 0);
console.log(totalPriceOfRiceFoods);
// THE TOTAL
36
